app.views.beta = Backbone.View.extend({
    template: _.template($("#tpl-page-beta").html()),
    events: {
    ...
    event_stats: function(event, count){
        ...
        $.post(app.appURL+'event_tweets_count', {obj: JSON.stringify(s_ev), index: app.session.s_index}, function(response){
            ...
            html +='<td>'+response.percentage+'</td>';
        ...
        // app.eventsCollection.each(function(event){
            ...
            // if(event.cid==="c3"){
                ...
                // self.get_elbow(res);
                 ...
                 var html = '<tr>' +
                        '    <td>' + att.main_term + '</td>' +
                        '    <td><div id="'+el+'" style="height: 370px; width: 750px;"></div></td>' +
                        '</tr>';
